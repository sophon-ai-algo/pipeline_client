...
public:
    ...
    int draw_rect(const fdrtsp::FaceRectVector& vct_rect,const std::vector<fdrtsp::FaceRecognitionInfo> &vct_label, bool bUpdate=true) override;
    int clear_frame();

public slots:
    void slot_draw_frame(const AVFrame *frame); //显示一帧Yuv图像
    void slot_draw_rect(const fdrtsp::FaceRectVector& vct_rect, const std::vector<fdrtsp::FaceRecognitionInfo> &vct_label);
